...
class RapidJsonAdapter {
 ...
 public:
  ...
  RapidJsonAdapter() { document_.SetObject(); }

  explicit RapidJsonAdapter(std::istream& stream) {
    rapidjson::IStreamWrapper isw(stream);
    document_.ParseStream(isw);
  }

  // For serialization - store stream reference
  explicit RapidJsonAdapter(std::ostream& stream) : writeStream_(&stream) { document_.SetObject(); }
...
// Convenience alias for RapidJSON serializable types
template <typename T>
using RapidJsonSerializable = Serializable<T, RapidJsonAdapter>;

}  // namespace serialization

// Global alias for easier access
template <typename T>
using RapidJsonSerializable = serialization::RapidJsonSerializable<T>;

}  // namespace lazy

#endif  // LAZY_SERIALIZATION_RAPID_JSON
